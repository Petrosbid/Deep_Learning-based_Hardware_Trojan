#
# train_nlp.py
# (Phase 2: Train Net2Vec Model using Gensim)
#
import os
import json
import gensim
...
# --- Settings ---
# (This file was created by preprocess_nlp.py)
CORPUS_FILE = "../jsonl_dataset/corpus_ALL.jsonl"

# Output model file (vector dictionary)
MODEL_OUTPUT_FILE = "../Model/net2vec.vectors"

# Model parameters, as per the paper
EMBEDDING_DIM = 100  # (N=100)
NEGATIVE_SAMPLES = 5  # (K=5)
WINDOW_SIZE = 9  # (2 * logic_level=5) - 1. Consider a large window
MIN_WORD_COUNT = 1  # Minimum number of repetitions of a word to be included
WORKERS = os.cpu_count() - 2  # Use all CPU cores except 2

# --- Set logging to show progress ---
logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)


class CorpusStreamer:
    """
    A class to read 48 million lines without loading into RAM.
    ...
    def __iter__(self):
        print("\n--- 🧠 Starting stream reading of Corpus... (this step may take a while) ---")
        try:
            with open(self.filepath, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
                        # Each line is a list (sentence)
...
def train_net2vec_model(corpus_filepath):
    ...
    # 1. Create an instance of CorpusStreamer
    ...
    print("This process will take several hours and use CPU...")

    # 2. Build and train model
    # gensim automatically streams data from 'sentences_stream'
    model = gensim.models.Word2Vec(
        sentences=sentences_stream,
        vector_size=EMBEDDING_DIM,
        sg=1,  # 1 = Skip-gram (as per paper)
        negative=NEGATIVE_SAMPLES,
        window=WINDOW_SIZE,
        min_count=MIN_WORD_COUNT,
        workers=WORKERS,
        epochs=5  # 5 full epochs over the entire dataset
    )
    ...
    try:
        ...
        # We only need the dictionary (KeyedVectors), not the whole model
    ...
    print(f"\n--- 📊 Model Results ---")
    print(f"Total number of unique words (PCPs) in dictionary: {vocab_size}")
    if vocab_size > 0:
...
def main():
    ...
    print("\n🏁 Phase 2 (NLP Training) completed.")
    print(f"Next step: Create balanced dataset (Downsampling) and train LSTM detector.")
