#
# detector.py
# (Final version with pin normalization and vocabulary debugging)
#
import os
import re
import time
...
# 1. Pin name mapping to standard names (to solve DIN1 vs A issue)
PIN_NORMALIZATION = {
    # Outputs
...
# Any other pin not listed here will be considered 'IN' if it's an input or 'OUT' if it's an output.
...
# 3. Manual settings dictionary (to be filled by user)
CUSTOM_GATE_DEFINITIONS = {}

# Whitelist of outputs
...
def run_interactive_setup():
    print("\n" + "=" * 60)
    print(" 🛠️  Manual Parser Configuration Section  🛠️")
    print("=" * 60)

    while True:
        choice = input("\n>> Do you need to manually define special gates? (y/n): ").strip().lower()
        if choice in ['y', 'n', 'yes', 'no']: break

    if choice in ['n', 'no']:
        print("✅ Using smart settings.")
        return

    while True:
        print("-" * 30)
        gate_name = input("1. Gate Type Name: ").strip()
        if not gate_name: continue
        out_str = input(f"2. Output pins of '{gate_name}' (comma separated): ").strip()
        outs = [x.strip() for x in out_str.split(',') if x.strip()]
        in_str = input(f"3. Input pins of '{gate_name}' (comma separated): ").strip()
...
class Gate:
    ...
    def _clean_type(self, raw_type):
        ...
        # 2. Exact mapping (priority given to complete mapping)
        if clean in TYPE_MAPPING:
            return TYPE_MAPPING[clean]

        # 3. Word start mapping
    ...
    def infer_pin_directions(self):
        ...
        if not temp_outputs:
            ...
            elif temp_inputs:  # Extreme fallback
...
def parse_netlist_dynamic(netlist_file_path: str) -> Optional[Netlist]:
    ...
    re_wire = re.compile(r'^\s*wire\s+(.+);', re.DOTALL)
    ...
    try:
        with open(netlist_file_path, 'r', encoding='utf-8') as f:
            content = f.read()
            # Remove comments to prevent errors
            content = re.sub(r'//.*', '', content)

            # Process statements ending with ;
...
# ------------------------------------------------------------------------------
#  Corrected Section: PCP Extraction with Pin Normalization
# ------------------------------------------------------------------------------
def extract_pcp_traces(netlist_blocks: Dict) -> Dict[str, List[List[str]]]:
    ...
    def get_normalized_pin_name(raw_pin: str) -> str:
        """Normalizes the pin name (DIN1 -> IN)"""
        # Pin name is in format gate___pin. We only want the pin
        pin_only = raw_pin.split('___')[-1].upper()

        # 1. Search in direct dictionary
        if pin_only in PIN_NORMALIZATION:
            return PIN_NORMALIZATION[pin_only]

        # 2. Search by prefix for DIN1, DIN2, etc.
        for key, val in PIN_NORMALIZATION.items():
            if pin_only.startswith(key):
                return val

        # 3. Default: if not found, guess based on its name
...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, word in enumerate(trace_words):
            ...
            # --- Try to find the word in the dictionary (Fuzzy Matching) ---
            found = False
            if word in self.embeddings:
                trace_tensor_data[i] = self.embeddings[word]
                found = True
            else:
                # If the exact word (like IN___nand2___OUT) is not present, check simplified forms
                # Sometimes the model is trained only on nand2 not on the complete PCP structure
        ...
        for trace in traces:
            for word in trace:
                ...
                if word in embeddings:
                    ...
                    # Check simplified form (gate name only)
                    parts = word.split('___')
