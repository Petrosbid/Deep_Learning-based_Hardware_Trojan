...
def find_files_to_delete(root_folders, patterns):
    """
    Recursively finds all files matching the patterns in the target folders.
    """
    files_to_delete = []
    ...
    for root in root_folders:
        ...
        for pattern in patterns:
            # Use glob to recursively search in all subdirectories
            search_pattern = os.path.join(root, '**', pattern)

            # recursive=True tells glob to search in all subdirectories (**)
...
def main():
    ...
    parser.add_argument(
        '--force',
        action='store_true',  # If this flag exists, sets the value to True
        help="Execute actual deletion. If not used, only lists the files."
    )
    ...
    print(f"Found {len(files_found)} files for processing.")
    print("-" * 50)

    # --- 3. Execute deletion or dry run ---
    deleted_count = 0
    failed_count = 0

    if args.force:
        print("⚠️ Warning: '--force' mode activated. Performing permanent file deletion...")
        time.sleep(2)  # A brief pause to read the warning
    ...
    if args.force:
        ...
        print(f"  ❌ Errors in deletion: {failed_count}")
    else:
