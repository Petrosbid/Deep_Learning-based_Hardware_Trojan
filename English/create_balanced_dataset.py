...
def main():
    ...
    print(f"--- 🏁 Starting creation of balanced dataset from {INPUT_FILE} ---")
    print("This process may take some time due to reading the 48 million dataset...")

    # ==================================================================
    #  Reading file to collect trojans and count clean data
    # ==================================================================
    print(f"\n--- Step 1 of 2: Collecting Trojan samples (Label 1)... ---")

    trojan_traces = []  # All trojan samples are stored here (in RAM)
    total_normal_count = 0  # Only count clean samples

    start_time_pass1 = time.time()

    try:
        with open(INPUT_FILE, 'r', encoding='utf-8') as f:
            # Using tqdm to show progress in step 1
            for line in tqdm(f, total=TOTAL_LINES, desc="📊 Step 1: Counting", unit="L"):
                try:
                    ...
                    tqdm.write(f"  ⚠️ Warning: Skipping malformed line: {line.strip()}")

    except Exception as e:
        print(f"\n❌ Error during step 1 read: {e}")
        return

    end_time_pass1 = time.time()
    k_trojans = len(trojan_traces)  # Total number of trojan samples

    print(f"--- ✅ Step 1 complete (in {end_time_pass1 - start_time_pass1:.2f} seconds) ---")
    print(f"  📈 {k_trojans:,} trojan samples (Label 1) found (stored in RAM).")
    print(f"  📉 {total_normal_count:,} clean samples (Label 0) counted.")

    if k_trojans == 0:
        print("❌ Error: No trojan samples (Label 1) found in your dataset. Process stopped.")
        return

    if total_normal_count == 0:
        print("❌ Error: No clean samples (Label 0) found in your dataset. Process stopped.")
        return

    # ==================================================================
    #  Downsampling clean samples
    # ==================================================================
    print(f"\n--- Step 2 of 2: Down-sampling {k_trojans:,} clean samples... ---")

    # Calculate downsampling rate
    # This is the same technique called "Downsampling" in the paper
    sampling_rate = k_trojans / total_normal_count
    print(f"  Sampling Rate for Label 0: {sampling_rate:.6f}")

    normal_traces_selected = []  # Selected clean samples are stored here
    start_time_pass2 = time.time()

    try:
        with open(INPUT_FILE, 'r', encoding='utf-8') as f:
            # Using tqdm to show progress in step 2
            for line in tqdm(f, total=TOTAL_LINES, desc="📊 Step 2: Sampling", unit="L"):
                try:
                    # We only care about clean samples in this step
                    if '"label": 0' not in line:
                        continue

                    # Random sampling method:
                    # Instead of loading everything, give each clean sample a small chance
        ...
        print(f"\n❌ Error during step 2 read: {e}")
        return

    end_time_pass2 = time.time()
    print(f"--- ✅ Step 2 complete (in {end_time_pass2 - start_time_pass2:.2f} seconds) ---")
    print(f"  📉 {len(normal_traces_selected):,} clean samples (Label 0) randomly selected.")

    # ==================================================================
    #  Merging, shuffling and saving
    # ==================================================================
    ...
    print(f"  - Clean traces (Label 0): {len(normal_traces_selected):,}")
