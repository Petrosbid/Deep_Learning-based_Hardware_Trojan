        ...
        with open(self.data_file, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="📊 Reading and filtering", unit="L"):
                try:
                    ...
                    tqdm.write(f"⚠️ Skipping corrupted line: {line_stripped[:50]}...")

        self.total_samples = len(self.data)
        if self.total_samples == 0:
            print("⚠️ Warning: 0 samples loaded after filtering. Are circuit names correct?")
        else:
...
if __name__ == "__main__":
    print("--- 🧪 Starting TrojanDataset Test (Final and Fixed Version) ---")
    try:
        print("\n--- Test Loading (No Filter) ---")
        ...
        print(f"❌ Unknown error during test: {e}")