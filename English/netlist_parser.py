...
TYPE_MAPPING = {
    ...
    'lo1': 'tielo',
    'assign': 'buffer',
    'ib1': 'buffer',  # Added: input buffer
    'i1': 'buffer',  # Added: regular buffer
    'and': 'and',  # To prevent unwanted changes
    'or': 'or',
    'nor': 'nor',
    'xor': 'xor'
}

# Pin direction detection settings
...
class Gate:
    ...
    def _clean_type(self, raw_type):
        ...
        # 2. Exact mapping (Exact Match has priority)
        # If clean is exactly the key, return the value
        if clean in TYPE_MAPPING:
            return TYPE_MAPPING[clean]

        # 3. Partial mapping carefully
        # Only if the word starts with the key (not in the middle)
